...
export default function UnstakedDesoTable() {
  ...
  const grouped = useMemo(() => {
    ...
    const sortedDesoBulls = [...desoBulls].sort((a, b) => b.usdValue - a.usdValue);
    return {
  ...
  function WalletRow({ w }: { w: (typeof allWallets)[0] }) {
    ...
    const unstaked = w.desoUnstaked ?? Math.max(0, (w.balances.DESO ?? 0) - (w.desoStaked ?? 0));
    return (
      <tr>
        ...
        <td className="text-xs text-muted-foreground">
          {(() => {
            ...
            for (const token of tokens) {
              ...
              if (amt <= 0) continue;
              if (token === 'DESO') {
                // Only show unstaked DESO (exclude staked)
                if (unstaked > 0) items.push({ key: 'DESO-unstaked', label: 'DESO', amt: unstaked });
              } else {
          ...
          <table className="data-table">
            <thead>
              <tr>
                <th>Wallet</th>
                <th>Class</th>
                <th>Key Holdings (excl. staked DESO)</th>
